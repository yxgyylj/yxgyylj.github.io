...
window.webkitRequestAnimationFrame || window.msRequestAnimationFrame
  ...
  '间隔群山峻岭', '你看不见我', '我看不见你', '但是 你看那',
   '满天的繁星', '都同样的闪耀', '就像我在你身边', '认识你 遇见你', '是一种缘分',
   '更是我的幸运','我想你','如果你也想我','请相信',
 '金风玉露一相逢', '便胜却人间无数',
 '两情若是久长时', '又岂在朝朝暮暮','爱你 -- 夕歌']
  ...
  let canvas,
    ...
    text = texts[0],
    textIndex = 0,
    textSize = Math.min(CANVASWIDTH,CANVASHEIGHT) / 10
  ...
  function draw () {
    ...
    ctx.fillText(text, (CANVASWIDTH - ctx.measureText(text).width) * 0.5, CANVASHEIGHT * 0.3)
  ...
  class Particle {
    constructor (canvas) {
      ...
      this.y = 0
      // 上次的位置
      this.px = Math.random() * canvas.width * 10
      this.py = (canvas.height * 0.5) + ((Math.random() - 0.5) *10* spread)
      // 记录点最初的位置
      this.mx = this.px
      this.my = this.py
      // 点的大小
